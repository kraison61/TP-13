{{-- Hero ตาม example/Services.dc.html — ใช้กับหน้า index (blog, services, galleries) --}}
<section class="relative overflow-hidden bg-navy-900 text-white">
  ...
  <div class="relative mx-auto max-w-7xl px-6 py-16 lg:py-20">
    <x-frontend.breadcrumb variant="dark" embedded :current="$current" class="mb-8" />
    <div class="max-w-3xl">
      @if ($eyebrow)
      ...
      <div class="mt-10 flex flex-wrap gap-3">
        @foreach ($badges as $badge)
          <span class="inline-flex items-center gap-2 rounded-full bg-white/10 ring-1 ring-white/20 px-4 py-2 text-[14px]">
            <i class="bi {{ $badge['icon'] }} text-hivis"></i> {{ $badge['text'] }}
          </span>
        @endforeach
      </div>
    @endif
