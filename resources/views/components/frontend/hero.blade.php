<section @class([
    ...
    'bg-gradient-to-b from-surface to-white' => $layout === 'home',
]) @if ($layout === 'home') id="home" @endif>
    ...
    <div @class([
        ...
        'py-14 sm:py-16 lg:py-24' => $layout === 'home',
        'py-16 lg:py-20' => $layout === 'page',
    ])>
        @if ($layout === 'home')
            <div class="grid lg:grid-cols-2 gap-10 sm:gap-12 lg:gap-16 items-center">
                <div>
                    ...
                    <p class="mt-5 text-lg leading-relaxed text-ink2 max-w-xl">
                        รัพวกเราคือผู้เชี่ยวชาญงานโครงสร้างงาน <strong class="text-navy-900 font-semibold">กำแพงกันดิน · รั้วบ้าน · ถนน · ลานคอนกรีต · และงานปรับหน้าดิน ถมดิน บดอัด</strong>
                        โดยมีสำนักงานใหญ่ตั้งอยู่ที่ จ.นนทบุรี พร้อมศักยภาพในการระดมทีมช่างประจำกว่า {{ config('company.team_size') }} คน
        ...
        @else
            <div class="max-w-3xl">
                @if ($eyebrow)
                    <span class="inline-flex items-center gap-2 text-accent font-semibold tracking-[0.18em] text-xs uppercase">
                        <span class="w-7 h-px bg-accent"></span> {{ $eyebrow }}
                    </span>
                @endif
                @if ($title)
                    <h1 class="mt-4 text-[clamp(2rem,4vw,3.2rem)] font-bold leading-[1.3] tracking-tight text-navy-900">{!! $title !!}</h1>
                @endif
                @if ($description)
                    <p class="mt-4 text-lg text-ink2 leading-relaxed max-w-2xl">{{ $description }}</p>
                @endif
            </div>
            @if ($badges)
